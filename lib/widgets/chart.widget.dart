...
class Chart extends StatelessWidget {
  ...
  List<Map<String, Object>> get groupedTransactionValues {
    return List.generate(7, (index){
      ...
      return {'day':DateFormat.E().format(weekDay).substring(0,1), 'amount':totalSpend};
    });
  }
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: Row(children: groupedTransactionValues.map((data) {
        return ChartBar(data['day'], data['amount'], totalMaxExpenses == 0 ? 0.0 : (data['amount'] as double) / totalMaxExpenses);
      }).toList(),),
    );
  }
}